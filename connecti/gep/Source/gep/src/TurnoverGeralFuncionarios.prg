// Proyecto: GEP
/*
 * Fichero: TurnOverGeralFuncionarios.prg
 * Descripción:
 * Autor:
 * Fecha: 14/08/2021
 */
...
#define __CODMODEL__ "TURNOVERGERALFUNCIONARIOS"

PROCEDURE ParametersTurnOverGeralFuncionarios( lGrpViewTotais )

   LOCAL hUser := LoginUser()

   LOCAL cSubTitle := " :: " + "Consulta TurnOver Geral por Funcionários"
   LOCAL cTitle := AppData:AppTitle + cSubTitle
   LOCAL cIDParameters
   LOCAL aParameters

   hb_default( @lGrpViewTotais, .F. )
   oCGI:SetUserData( "__TurnOverGeralFuncionarios:lGrpViewTotais", lGrpViewTotais )
   ...
   WITH OBJECT wTWebPage():New()
      ...
      GEPParameters( :WO, @cIDParameters, "__TurnOverGeralFuncionarios", aParameters, .T., cSubTitle, .T. ):Create()
...
FUNCTION __TurnOverGeralFuncionarios(lExecute,hFilter,nMaxRowspPage)
   ...
   IF (stacktools():IsInCallStack("TCGI:__TurnOverGeralFuncionariosDet"))
      deleteTmpParameters("__TurnOverGeralFuncionarios")
   ELSE
      restoreTmpParameters("__TurnOverGeralFuncionarios",@hFilter,.T.)
   ENDIF

   IF ( lExecute )
      cIDModal := oCGI:GetCgiValue( "cIDModalTotaisTurnOverGeralFuncionarios", "" )
      IF ( !Empty( cIDModal ) )
         oCGI:SetUserData( "HistoryRemoveParams", .F. )
         cIDModal := __base64Decode( cIDModal )
         hb_jsonDecode( cIDModal, @hIDModal )
         saveTmpParameters( "__TurnOverGeralFuncionarios", hIDModal )
   ...
   IF (Empty(hFilter[ "codFilial" ]))
      hFilter:=HB_HDel(hFilter, "codFilial" )
   ENDIF
   IF (Empty(hFilter[ "codCentroDeCusto" ]))
      hFilter:=HB_HDel(hFilter, "codCentroDeCusto" )
   ENDIF
   IF (Empty(hFilter[ "codFuncao" ]))
      hFilter:=HB_HDel(hFilter, "codFuncao" )
   ENDIF

   saveTmpParameters( "__TurnOverGeralFuncionarios",hFilter)

   IF ( lExecute )
      TurnOverGeralFuncionarios(nMaxRowspPage)
   ENDIF

RETURN( hFilter )

PROCEDURE TurnOverGeralFuncionarios(nMaxRowspPage)
   ...
   IF ( !stackTools():IsInCallStack( "__TurnOverGeralFuncionarios" ) )
      hFilter:=__TurnOverGeralFuncionarios( .F. )
   ENDIF
   ...
   WITH OBJECT wTWebPage():New()
      ...
      codPeriodo := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codPeriodo" )
      codPeriodoAte := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codPeriodoAte" )
      ...
      ELSE
         ...
         GEPParameters( :WO, @cIDParameters, "__TurnOverGeralFuncionariosDet", aParameters, .F., cSubTitle ):Create()

         :lValign    := .F.
         :lContainer := .F.

         aFields := GetDataFieldsTurnOverGeralFuncionarios( @cColsPrint )
         ...
         WITH OBJECT owDataTable:=wDatatable():New( :WO )
            ...
            :cId := Lower("browseTurnOverGeralFuncionarios")
                     ...
                     [ 'csv',
                        {
                           ...
                           customize: function(xlsx) {
                              ...
                              $('row:gt(1) c[r^="G"]',sheet).attr('s','50');
                              $('row:gt(1) c[r^="H"]',sheet).attr('s','50');
                              //Date Value
                              $('row:gt(1) c[r^="I"]',sheet).attr('s','67');
                              $('row:gt(1) c[r^="J"]',sheet).attr('s','67');
                           }
                        },
                        {
                           extend: 'pdfHtml5',
                           ...
                           customize : function(doc) {
                              ...
                              for (c = 8; c < doc.content[1].table.body[0].length; c++) { // right align headers for numeric cols
                                 doc.content[1].table.body[0][c].alignment = 'right';
                              }
                              for (i = 1; i < rowCount; i++) {
                                  for (c = 8; c < doc.content[1].table.body[i].length; c++) { // right align numeric cols
...
*               :AddColumnDef( nField ):width := '14px'
               ...
               :ajax := { "cgi" => "GetDataTurnOverGeralFuncionarios" }

            END

            AAdd( :aScript, fixedColumns( :cID,oCGI:GetUserData( __CODMODEL__+":fixedColumns","8" ) ) )
...
FUNCTION GetDataTurnOverGeralFuncionarios( lSendJSON, lGetFull, cSearchFilter )
   ...
   IF ( !nLimit == 0 )
      GetDataFieldsTurnOverGeralFuncionarios()
...
FUNCTION GetDataFieldsTurnOverGeralFuncionarios( cColsPrint )
   local aFields
   local codModel:=__CODMODEL__
...
STATIC FUNCTION Extraer( codModel, nPage, nRecords, nDraw, cSearchFilter, aOrder, lSendJSON, lGetFields )
   ...
   codFilial := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codFilial" )
   codFuncao := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codFuncao" )
   codPeriodo := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codPeriodo" )
   codPeriodoAte := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codPeriodoAte" )
   codCentroDeCusto := oCGI:GetUserData( "__TurnOverGeralFuncionarios:codCentroDeCusto" )
...
PROCEDURE __clearTurnOverGeralFuncionarios()

   oCGI:SetUserData( "__TurnOverGeralFuncionarios:codFilial", "" )
   oCGI:SetUserData( "__TurnOverGeralFuncionarios:codFuncao", "" )
   oCGI:SetUserData( "__TurnOverGeralFuncionarios:codPeriodo", "" )
   oCGI:SetUserData( "__TurnOverGeralFuncionarios:codPeriodoAte", "" )
   oCGI:SetUserData( "__TurnOverGeralFuncionarios:codCentroDeCusto", "" )

   oCGI:SetUserData( "TurnOverGeralFuncionarios:Back", "ParametersTurnOverGeralFuncionarios" )
