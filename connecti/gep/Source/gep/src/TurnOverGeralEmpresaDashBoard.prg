...
PROCEDURE __TurnOverGeralEmpresaDashBoard()
   ...
   WITH OBJECT oTWebPage:=wTWebPage():New()
      ...
      WITH object wDashBoardTurnOver():New( oTWebPage )
         ...
         :totalDemissoes:=0
         :totalFuncIMes:=0
         :totalFuncIPer:=Val(hData[ "data" ][ 1 ][ "totalFuncionariosInicioMes" ])
         nRows := Len( hData[ "data" ] )
         FOR nRow := 1 TO nRows
            AAdd(:transfEntData,Val(hData[ "data" ][ nRow ][ "totalTransferenciasEntrada" ]))
            AAdd(:transfSaiData,Val(hData[ "data" ][ nRow ][ "totalTransferenciasSaida" ]))
            AAdd(:admissoesData,Val(hData[ "data" ][ nRow ][ "totalFuncionariosAdmitidos" ]))
            AAdd(:demissoesData,Val(hData[ "data" ][ nRow ][ "totalFuncionariosDemitidos" ]))
            AAdd(:totalfuncIMesData,Val(hData[ "data" ][ nRow ][ "totalFuncionariosInicioMes" ]))
            AAdd(:totalfuncFMesData,Val(hData[ "data" ][ nRow ][ "totalFuncionariosFinalMes" ]))
            AAdd(:turnOverGeralData,Val(hData[ "data" ][ nRow ][ "turnOverGeral" ]))
            AAdd(:turnOverDemissoesData,Val(hData[ "data" ][ nRow ][ "turnOverDemissoes" ]))
            :totalFuncIMes+=Val(hData[ "data" ][ nRow ][ "totalFuncionariosInicioMes" ])
            :totalAdmissoes+=Val(hData[ "data" ][ nRow ][ "totalFuncionariosAdmitidos" ])
            :totalDemissoes+=Val(hData[ "data" ][ nRow ][ "totalFuncionariosDemitidos" ])
         NEXT nRow
         :totalFuncFPer:=Val(hData[ "data" ][ --nRow ][ "totalFuncionariosFinalMes" ])
