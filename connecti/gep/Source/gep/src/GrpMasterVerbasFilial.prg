// Proyecto: GEP
/*
 * Fichero: GrpMasterVerbasFilial.prg
 * Descripción:
 * Autor:
 * Fecha: 14/08/2021
 */
...
#define __CODMODEL__ "GRPMASTERVERBASFIL"

PROCEDURE ParametersGrpMasterVerbasFilial( lGrpViewTotais )

   LOCAL hUser := LoginUser()

   LOCAL cSubTitle:=" :: "+"Consulta Dados Por Filial/Grupo de Verbas"
   ...
   WITH OBJECT wTWebPage():New()
      ...
      GEPParameters( :WO,@cIDParameters,"__GrpMasterVerbasFilial",aParameters,.T.,cSubTitle,.T. ):Create()
...
function __GrpMasterVerbasFilial(lExecute,hFilter,nMaxRowspPage)
   ...
   IF (lExecute)
      ...
      IF (!Empty(cIDModal))
         ...
         saveTmpParameters("__GrpMasterVerbasFilial",hIDModal)
      ...
      ELSE
         IF (HB_HHasKey(hFilter,"codGrupo"))
            ...
            oCGI:SetUserData("__GrpMasterVerbasFilial:codGrupo",codGrupo)
         ELSE
         ...
         IF (HB_HHasKey(hFilter,"codPeriodo"))
            ...
            oCGI:SetUserData("__GrpMasterVerbasFilial:codPeriodo",codPeriodo)
         ELSE
   ...
   if (lExecute)
      ...
      GrpMasterVerbasFilial(nMaxRowspPage)
   endif

   RETURN(hFilter)

PROCEDURE GrpMasterVerbasFilial(nMaxRowspPage)
   ...
   WITH OBJECT oTWebPage:=wTWebPage():New()
      ...
      aParameters := { "codPeriodo","codGrupo","codFilial" }
      cSubTitle:=AppData:AppName+cSubTitle
      GEPParameters(oTWebPage,@cIDParameters,"__GrpMasterVerbasFilial",aParameters,.F.,cSubTitle ):Create()

      :lValign    := .F.
      :lContainer := .F.

      aFields := GetDataFieldsGrpMasterVerbasFilial( @cColsPrint )
      ...
      WITH OBJECT owDatatable:=wDatatable():New(oTWebPage)
         ...
         :cId := Lower("browseGrpMasterVerbasFilial")
        ...
        if (!lGrpViewTotais)
            ...
            if (lGrpViewTotais)
               hFilter:=__GrpMasterVerbasFilial(.F.,hFilter)
               __TotaisGrpMasterVerbasFilial(owDatatable,.F.,.F.)
         ...
         WITH OBJECT :Configure()
            ...
            :AddColumnButton( "loupe",cTmp,"btn-wDatatable ;","__GrpMasterVerbasCentroDeCustoDet","dt-center","",/*,'fn_click'*/ )
            :AddColumnDef( nField ):width := '30px'

            :ajax := { "cgi" => "GetDataGrpMasterVerbasFilial" }
...
FUNCTION GetDataGrpMasterVerbasFilial( lSendJSON,lGetFull,cSearchFilter )
   ...
   IF ( !nLimit == 0 )
      GetDataFieldsGrpMasterVerbasFilial()
...
FUNCTION GetDataFieldsGrpMasterVerbasFilial( cColsPrint )
...
STATIC FUNCTION Extraer( codModel,nPage,nRecords,nDraw,cSearchFilter,aOrder,lSendJSON,lGetFields )
   ...
   hFilter:={=>}
   restoreTmpParameters("__GrpMasterVerbasFilial",@hFilter,.T.)

   codGrupo := oCGI:GetUserData( "__GrpMasterVerbasFilial:codGrupo" )
   codFilial := oCGI:GetUserData( "__GrpMasterVerbasFilial:codFilial" )
   codPeriodo := oCGI:GetUserData( "__GrpMasterVerbasFilial:codPeriodo" )
...
PROCEDURE __clearGrpMasterVerbasFilial()

   oCGI:SetUserData( "__GrpMasterVerbasFilial:codGrupo","" )
   oCGI:SetUserData( "__GrpMasterVerbasFilial:codFilial","" )
   oCGI:SetUserData( "__GrpMasterVerbasFilial:codPeriodo","" )

   oCGI:SetUserData("GrpMasterVerbasFilial:Back:lGrpViewTotais",.F.)
