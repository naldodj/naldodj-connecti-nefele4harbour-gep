// Proyecto: GEP
/*
 * Fichero: PainelGrpMasterVerbasFuncionarios.prg
 * Descripción:
 * Autor:
 * Fecha: 14/08/2021
 */
...
#define __CODMODEL__ "PIVOTFUNCIONARIOGRUPO"

PROCEDURE ParametersPainelGrpMasterVerbasFuncionarios( lGrpViewTotais )
   ...
   WITH OBJECT wTWebPage():New()
      ...
      GEPParameters( :WO,@cIDParameters,"__PainelGrpMasterVerbasFuncionarios",aParameters,.T.,cSubTitle,.T. ):Create()
...
FUNCTION __PainelGrpMasterVerbasFuncionarios(lExecute,hFilter,nMaxRowspPage)
   ...
   IF ( lExecute )
      ...
      IF ( !Empty( cIDModal ) )
         ...
         saveTmpParameters( "__PainelGrpMasterVerbasFuncionarios", hIDModal )
      ...
      ELSE
         ...
         IF ( hb_HHasKey( hFilter,"codCentroDeCusto" ) )
            ...
            oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codCentroDeCusto", codCentroDeCusto )
         ELSE
         ...
         IF ( hb_HHasKey( hFilter,"codGrupoSuperior" ) )
            ...
            oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codGrupoSuperior", codGrupoSuperior )
         ELSE
   ...
   IF ( oCGI:GetUserData( "lGrupoHasSuper", .F. ) )
      hFilter[ "codGrupo" ] := ""
      oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codGrupo", "" )
      hFilter[ "codGrupoSuperior" ] := codGrupo
      oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codGrupoSuperior", codGrupo )
   ELSE
   ...
   IF ( lExecute )
      ...
      PainelGrpMasterVerbasFuncionarios(nMaxRowspPage)
   ENDIF

RETURN( hFilter )

PROCEDURE PainelGrpMasterVerbasFuncionarios(nMaxRowspPage)

   LOCAL hUser := LoginUser()

   LOCAL cSubTitle:=" :: "+"Painel Consulta Dados Por Centro de Custo/Funcionario/Grupo de Verbas"
   LOCAL cTitle := AppData:AppTitle+cSubTitle
   LOCAL cColsPrint
   LOCAL cIDParameters
   LOCAL aParameters
   ...
   IF ( !stackTools():IsInCallStack( "__PainelGrpMasterVerbasFuncionarios" ) )
      hFilter:=__PainelGrpMasterVerbasFuncionarios( .F. )
   ENDIF
   ...
   WITH OBJECT wTWebPage():New()
      ...
      cSubTitle:=AppData:AppName+cSubTitle
      GEPParameters( :WO,@cIDParameters,"__PainelGrpMasterVerbasFuncionarios",aParameters,.F.,cSubTitle ):Create()

      :lValign    := .F.
      :lContainer := .F.

      aFields := GetDataFieldsPainelGrpMasterVerbasFuncionarios( @cColsPrint )
      ...
      WITH OBJECT wDatatable():New( :WO )
         ...
         :cId := Lower("browsepainelGrpMasterVerbasFuncionarios")
         ...
         WITH OBJECT :Configure()
            ...
            :AddColumnButton( "loupe",cTmp,"btn-wDatatable ;",IF(__objHasMethod(oCGI,"__GrpMasterVerbasFuncionariosDet"),"__GrpMasterVerbasFuncionarios","__GrpVerbasFuncionarios"),"dt-center","",/*,'fn_click'*/ )
            :AddColumnDef( nField ):width := '30px'

            :ajax := { "cgi" => "GetDataPainelGrpMasterVerbasFuncionarios" }
...
FUNCTION GetDataPainelGrpMasterVerbasFuncionarios( lSendJSON,lGetFull,cSearchFilter )
   ...
   IF ( !nLimit == 0 )
      GetDataFieldsPainelGrpMasterVerbasFuncionarios()
...
FUNCTION GetDataFieldsPainelGrpMasterVerbasFuncionarios( cColsPrint )
...
STATIC FUNCTION Extraer( codModel,nPage,nRecords,nDraw,cSearchFilter,aOrder,lSendJSON,lGetFields )
   ...
   hFilter:={=>}
   restoreTmpParameters("__PainelGrpMasterVerbasFuncionarios",@hFilter,.T.)

   codVerba:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codVerba")
   codGrupo:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codGrupo")
   codFilial:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codFilial")
   codFuncao:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codFuncao")
   codPeriodo:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codPeriodo")
   codMatricula:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codMatricula")
   codCentroDeCusto:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codCentroDeCusto")
   codGrupoSuperior:=oCGI:GetUserData("__PainelGrpMasterVerbasFuncionarios:codGrupoSuperior")
...
PROCEDURE __clearPainelGrpMasterVerbasFuncionarios()

   oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codGrupo", "" )
   oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codFilial", "" )
   oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codPeriodo", "" )
   oCGI:SetUserData( "__PainelGrpMasterVerbasFuncionarios:codCentroDeCusto", "" )

   oCGI:SetUserData( "GrpMasterVerbasFuncionarios:Back", "GrpMasterVerbasFuncionarios" )
   oCGI:SetUserData( "PainelGrpMasterVerbasFuncionarios:Back", "ParametersPainelGrpMasterVerbasFuncionarios" )

   deleteTmpParameters("__PainelGrpMasterVerbasFuncionarios")
