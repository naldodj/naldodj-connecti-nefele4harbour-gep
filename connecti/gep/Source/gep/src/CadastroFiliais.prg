...
#define __CODMODEL__ "FILIAIS_SM0_SRD"

function __CadastroFiliais(lExecute,hFilter,nMaxRowspPage)
   ...
   if (lExecute)
      CadastroFiliais(nMaxRowspPage)
   endif

   RETURN(hFilter)

PROCEDURE CadastroFiliais(nMaxRowspPage)

   local hUser := LoginUser()

   local cSubTitle:=" :: "+"Cadastro de Filiais"
   ...
   WITH OBJECT wTWebPage():New()
     ...
     END WITH
      ...
      aParameters:={"codFilial"}
      cSubTitle:=AppData:AppName+cSubTitle
      GEPParameters(:WO,@cIDParameters,"__CadastroFiliais",aParameters,.F.,cSubTitle):Create()
      ...
      *aAdd(:aScript,'function fn_click(e){ if( document.querySelector("#datas").offsetTop > window.innerHeight) {document.getElementById("datas").scrollIntoView()}     }')
      ...
      aFields:=GetDataFieldsCadastroFiliais(@cColsPrint)
      ...
      WITH OBJECT owDataTable:=wDatatable():New(:WO)
         ...
         :cId:=Lower("browsecadastrofiliais")
...
*            :AddColumnDef( nField ):width := '30px'
            ...
            :ajax := { "cgi" => "GetDataCadastroFiliais" }
...
FUNCTION GetDataCadastroFiliais(lSendJSON,lGetFull,cSearchFilter)
...
function GetDataFieldsCadastroFiliais(cColsPrint)
   local aFields
   local codModel:=__CODMODEL__
...
STATIC FUNCTION Extraer(codModel,nPage,nRecords,nDraw,cSearchFilter,aOrder,lSendJSON,lGetFields)
   ...
   IF (!stacktools():IsInCallStack("GEPParameters"))
      ...
      restoreTmpParameters("__CadastroFiliais",@hFilter,.T.)

      codFilial:=oCGI:GetUserData("__CadastroFiliais:codFilial")
...
procedure __clearCadastroFiliais()

   oCGI:SetUserData("__CadastroFiliais:codFilial","")

   oCGI:SetUserData("CadastroFiliais:Back","MAINFUNCTION")

   deleteTmpParameters("__CadastroFiliais")
